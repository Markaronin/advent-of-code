...
impl HeightMap {
    ...
    fn from_lines(lines: Vec<String>) -> Self {
        let mut new_height_map = HeightMap {
            data: vec![]
        };
    ...
    fn is_low_point(&self, coord: &Coordinate) -> bool {
        let surrounding_coordinates = coord.get_surrounding_coordinates(self.data.len(), self.data[0].len());
    ...
    fn get_basin_size(&self, low_point: &Coordinate) -> usize {
        ...
        while basin_queue.len() > 0 {
            ...
            let surrounding_coords = queue_coord
                .get_surrounding_coordinates(self.data.len(), self.data[0].len());
...
fn get_program_output(input_file: &str) -> (usize, usize) {
    ...
    let risk_levels_sum = low_points.iter().map(|coord| height_map.get_risk_level(coord)).sum();
    let mut all_basins = low_points.iter().map(|coord| height_map.get_basin_size(coord)).collect::<Vec<usize>>();
    all_basins.sort();
